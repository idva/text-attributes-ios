...
class LayoutManager: NSLayoutManager {
    ...
    override func drawGlyphs(forGlyphRange glyphRange: NSRange, at origin: CGPoint) {
        ...
        // Дополнительно вызываем метод для отрисовки подчеркивания (на случай, если нужно отриовать baseline например)
        ...
        if self.showBoundingRect {
            for i in glyphRange.location ..< NSMaxRange(glyphRange) {
                ...
                if let textContainer = self.textContainer(forGlyphAt: i, effectiveRange: nil) {
